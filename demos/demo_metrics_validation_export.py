...
def create_enhanced_demo_society(n_agents=6):
    ...
    agent_configs = [
        {
            "type": NeuralProcessingType.SYSTEMATIC,
            "personality": {
                "openness": 0.8,
        ...
        {
            "type": NeuralProcessingType.INTUITIVE,
            "personality": {
                "openness": 0.4,
        ...
        {
            "type": NeuralProcessingType.ASSOCIATIVE,
            "personality": {
                "openness": 0.6,
        ...
        {
            "type": NeuralProcessingType.ANALOGICAL,
            "personality": {
                "openness": 0.9,
        ...
        {
            "type": NeuralProcessingType.EMOTIONAL,
            "personality": {
                "openness": 0.3,
        ...
        {
            "type": NeuralProcessingType.NARRATIVE,
            "personality": {
                "openness": 0.7,
                "conscientiousness": 0.7,
                "extroversion": 0.6,
...
def run_comprehensive_simulation(society, n_scenarios=15):
    ...
    scenario_templates = ["trolley_problem", "autonomous_vehicle", "privacy_vs_security"]
    ...
    for step in range(n_scenarios):
        ...
        if step < len(scenario_templates):
            scenario = scenario_gen.create_scenario_from_template(
                scenario_templates[step % len(scenario_templates)]
            )
        else:
...
def run_comprehensive_validation(society):
    ...
    for val_type, stats in validation_summary["by_type"].items():
        if stats["count"] > 0:
            print(
                f"    - {val_type}: {stats['passed']}/{stats['count']} passed (avg: {stats['avg_score']:.3f})"
            )
...
def demonstrate_export_features(society, metrics_collector, validation_suite):
    ...
    summary = {
        ...
        "total_decisions": sum(
            len(getattr(agent, "decision_history", [])) for agent in society.agents.values()
        ),
...
def demonstrate_advanced_analysis(society, metrics_collector):
    ...
    if len(agents_list) >= 2:
                    ...
                    if agent1.beliefs and agent2.beliefs:
                        ...
                        if common_beliefs:
                            distance = np.mean(
                                [
                                    abs(
                                        agent1.beliefs[belief].strength
                                        - agent2.beliefs[belief].strength
                                    )
                                    for belief in common_beliefs
                                    if hasattr(agent1.beliefs[belief], "strength")
                                    and hasattr(agent2.beliefs[belief], "strength")
                                ]
                            )
        ...
        if belief_distances:
            ...
            print(
                f"    🤝 Convergence rate: {convergence_rate:.1%} ({converged_pairs}/{total_pairs} pairs)"
            )
...
def main():
    ...
    try:
        ...
        print(
            f"  - Total decisions: {sum(len(getattr(agent, 'decision_history', [])) for agent in society.agents.values())}"
        )
        ...
        print(
            f"  - Average metric score: {metrics_summary.get('metric_types', {}).get('overall', {}).get('mean', 'N/A')}"
        )
        print(
            f"  - Validation pass rate: {validation_summary['passed_checks']}/{validation_summary['total_checks']} ({validation_summary['passed_checks']/validation_summary['total_checks']:.1%})"
        )
