...
class PerformanceBenchmark:
    ...
    def run_full_benchmark(self):
        ...
        print(
            f"💻 System: {psutil.cpu_count()} CPUs, {psutil.virtual_memory().total / 1024 / 1024 / 1024:.1f}GB RAM"
        )
        print()
    ...
    def print_summary(self):
        ...
        slow_operations = [name for name, data in self.results.items() if data["time"] > 2.0]

        if slow_operations:
            print(f"  🔧 Langsame Operationen optimieren: {', '.join(slow_operations)}")

        memory_intensive = [name for name, data in self.results.items() if data["memory_mb"] > 50]
