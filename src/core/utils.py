...
def validate_belief_connections(
...
def load_config(config_file: str = "config.json") -> Dict[str, Any]:
    ...
    default_config = {
        "simulation": {"random_seed": 42, "max_agents": 100, "max_scenarios": 50},
        "logging": {"level": "INFO", "file": None},
        "export": {"auto_export": False, "export_format": "json"},
    }
    ...
    if os.path.exists(config_file):
        try:
            ...
            logging.warning(
                f"Fehler beim Laden der Konfiguration: {e}. Verwende Standard-Konfiguration."
            )
