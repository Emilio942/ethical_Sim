...
class PlotlyWebVisualizations:
    ...
    def create_interactive_network_3d(self):
        ...
        for i, node in enumerate(G.nodes()):
            ...
            if agent:
                ...
                node_text.append(
                    f"Agent: {node}<br>Personality: {personality}<br>Decisions: {decisions}"
                )
        ...
        node_trace = go.Scatter3d(
            ...
            hoverinfo="text",
            text=node_text,
            marker=dict(size=10, color=node_colors, line=dict(width=2, color="rgba(50,50,50,0.5)")),
            name="Agents",
        )
    ...
    def create_belief_evolution_timeline(self):
        ...
        fig.update_layout(height=400, showlegend=True, title_font_size=16)
        ...
        try:
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    gauge={"axis": {"range": [0, 1]}, "bar": {"color": "darkorange"}},
                ),
                row=2,
                col=2,
            )
            ...
            fig.update_layout(
                height=600, title_text="Real-time Metrics Dashboard", title_font_size=16
            )
    ...
    def create_agent_comparison_radar(self):
        ...
        for agent in self.society.agents[:6]:  # Limitiere auf 6 Agenten für Übersichtlichkeit
            ...
            # Überzeugungen
            beliefs_count = (
                len(getattr(agent.beliefs, "beliefs", {})) if hasattr(agent, "beliefs") else 0
            )
            values.append(min(beliefs_count / 20, 1))

            # Soziale Verbindungen
            connections = (
                self.society.social_network.degree(agent.agent_id)
                if agent.agent_id in self.society.social_network
                else 0
            )
