...
class EthicalSimulationVisualizer:
    ...
    def plot_agent_personalities(self, agents: List[Any], save_path: Optional[str] = None) -> None:
        ...
        personality_data = {trait: [] for trait in ['openness', 'conscientiousness', 'extroversion', 'agreeableness', 'neuroticism']}
        agent_names = []
...
"""
    ...
    def plot_scenario_decisions(self, scenario: Any, decisions: Dict[str, Dict], 
        ...
        fig.suptitle(f'Entscheidungsanalyse: {scenario.scenario_id}', fontsize=16, fontweight='bold')
        ...
        wedges, texts, autotexts = ax1.pie(option_counts.values(), 
...
"""
        ...
        ax3.text(0.05, 0.95, scenario_text, transform=ax3.transAxes, 
        ...
        for i, ptype in enumerate(unique_types):
            for j, option in enumerate(options):
                count = sum(1 for k, (at, dec) in enumerate(zip(agent_types, decision_list)) 
        ...
        if len(unique_types) > 0 and len(options) > 0:
            ...
            for i in range(len(unique_types)):
                for j in range(len(options)):
                    text = ax4.text(j, i, int(decision_type_matrix[i, j]),
    ...
    def plot_belief_evolution(self, agent: Any, belief_history: Dict[str, List[float]], 
                             save_path: Optional[str] = None) -> None:
        """
        ...
        for belief_name, values in belief_history.items():
            ...
            padded_values = values + [values[-1]] * (max_length - len(values)) if values else [0] * max_length
            ax1.plot(time_steps, padded_values, marker='o', linewidth=2, 
                    label=belief_name, alpha=0.8)
        ...
        if hasattr(agent, 'beliefs') and agent.beliefs:
            ...
            bars = ax2.bar(range(len(belief_names)), belief_values, 
            ...
            ax2.text(0.5, 0.5, 'Keine Überzeugungsdaten verfügbar', 
    ...
    def create_simulation_dashboard(self, society: Any, scenario: Any, decisions: Dict[str, Dict],
                                  save_path: Optional[str] = None) -> None:
        """
        ...
        if G.number_of_nodes() > 0:
            ...
            nx.draw(G, pos, ax=ax1, node_color=node_colors, node_size=500,
        ...
        if sum(option_counts.values()) > 0:
            colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7']
            ax2.pie(option_counts.values(), 
                   labels=[opt.replace('_', ' ').title() for opt in option_counts.keys()],
                   autopct='%1.1f%%', colors=colors[:len(options)])
        ...
        traits = list(personality_data.keys())
        avg_values = [np.mean(personality_data[trait]) if personality_data[trait] else 0 for trait in traits]
        
        x_pos = np.arange(len(traits))
        bars = ax3.bar(x_pos, avg_values, color=[self.personality_colors.get(trait, '#95A5A6') for trait in traits])
...
Agenten: {len(society.agents)} | Netzwerk-Dichte: {nx.density(society.social_network):.3f} | Entscheidungen: {len(decisions)}
        ...
        ax5.text(0.5, 0.5, info_text, transform=ax5.transAxes, ha='center', va='center',
