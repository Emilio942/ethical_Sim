...
class NeuralEthicalAgent:
                        ...
                        for concept, strength in belief.associated_concepts.items():
                            ...
                            for other_name, other_belief in self.beliefs.items():
                                if concept in other_belief.associated_concepts:
                                    assoc_strength = (
                                        strength * other_belief.associated_concepts[concept]
                                    )
    ...
    def update_from_social_influence(
        self, other_agent: "NeuralEthicalAgent", influenced_beliefs: List[str] = None
    ) -> Dict[str, float]:
        """
        ...
        social_learning_rate = (
            base_social_learning_rate * social_factor * agreeableness_factor * connection_strength
        )
        ...
        if influenced_beliefs is None:
            ...
            common_beliefs = (
                set(influenced_beliefs)
                .intersection(set(self.beliefs.keys()))
                .intersection(set(other_agent.beliefs.keys()))
            )
            ...
            self.episodic_memory.append(
                {
                    ...
                    "significant_changes": {
                        k: v for k, v in belief_changes.items() if abs(v) > 0.05
                    },
                }
            )
    ...
    def reflect_on_experiences(self) -> Dict[str, float]:
        ...
        if (
            self.cognitive_architecture.primary_processing != NeuralProcessingType.SYSTEMATIC
            and self.cognitive_architecture.secondary_processing != NeuralProcessingType.SYSTEMATIC
        ):
            return {}
        ...
        recent_memories = sorted(self.episodic_memory, key=lambda x: x["time"], reverse=True)[
            :memory_count
        ]
        ...
        for belief_name, count in most_common_beliefs:
            if belief_name in self.beliefs:
                ...
                for memory in recent_memories:
                    ...
                    elif memory["type"] == "social_influence" and belief_name in memory.get(
                        "significant_changes", {}
                    ):
                ...
                consistency = (
                    abs(avg_change) / (sum(abs(c) for c in changes) / len(changes))
                    if changes
                    else 0
                )
