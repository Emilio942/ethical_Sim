...
import os
import joblib
from typing import List, Dict, Tuple, Optional, Union, Callable, Any, cast, Any
...
class NeuralEthicalAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            cognitive_architecture: Optionale injizierte kognitive Architektur (Dependency Injection)
        ...
        if not agent_id or not isinstance(agent_id, str) or agent_id.strip() == "":
            logger.error("Versuch, Agent ohne gültige ID zu erstellen")
            raise ValueError("agent_id must be a non-empty string")

        self.agent_id = agent_id.strip()
        logger.debug(f"Initialisiere Agent: {self.agent_id}")
        self.beliefs: Dict[str, NeuralEthicalBelief] = {}  # name -> NeuralEthicalBelief
        ...
        # Kognitive Architektur (Dependency Injection oder Factory-Methode)
        if cognitive_architecture:
            self.cognitive_architecture = cognitive_architecture
            logger.debug(f"Verwende injizierte kognitive Architektur für Agent {self.agent_id}")
        else:
            self.cognitive_architecture = self._generate_cognitive_architecture()
            logger.debug(f"Generiere neue kognitive Architektur für Agent {self.agent_id}")
        ...
        # Gruppenidentitäten
        self.group_identities: Dict[str, float] = {}  # group_name -> identification_strength
        ...
        self.working_memory: Dict[str, Any] = {
            ...
            "retention": 0.7
            + 0.3 * self.personality_traits["conscientiousness"],  # Beibehaltungsrate
        }
        ...
        self.current_time = 0.0  # Simulationszeit für Aktivierungsdynamik

        # === ERWEITERTE LERNMECHANISMEN ===

        # Reinforcement Learning Komponenten
        self.rl_system: Dict[str, Any] = {
        ...
        self.uncertainty_handling: Dict[str, Any] = {
            "ambiguity_tolerance": self.personality_traits.get("openness", 0.5),
            "information_seeking_tendency": 0.5
            + 0.3 * self.personality_traits.get("conscientiousness", 0.5),
        ...
        # Temporale Dynamik
        self.temporal_dynamics: Dict[str, Any] = {
        ...
        self.metacognition: Dict[str, Any] = {
            "thinking_about_thinking": self.personality_traits.get("conscientiousness", 0.5) > 0.7,
            "strategy_monitoring": 0.4
            + 0.4 * self.personality_traits.get("conscientiousness", 0.5),
        ...
        # === AUDITIVE WAHRNEHMUNG (NEU) ===
        # Integration des auditiven Kortex (Phase 5.2)
        self.auditory_cortex = None
        self.auditory_features = None
        self.audio_loader = None
        self.emotion_model = None
        self.emotion_scaler = None
        self.emotion_labels = None
        
    def initialize_auditory_system(self):
        """
        Initialisiert das auditive System und lädt trainierte Modelle.
        """
        try:
            # 1. Initialisiere Cortex (wie im Demo)
            self.auditory_features = AuditoryFeatures(sr=16000, num_channels=64)
            self.audio_loader = AudioLoader(target_sr=16000)
            
            self.auditory_cortex = AuditoryCortex(num_modules=16, num_channels=64)
            strf_gen = STRFGenerator(num_channels=64, time_steps=20)
            self.auditory_cortex.initialize_filters(strf_gen)
            
            # 2. Lade trainierte Modelle
            # Pfad relativ zu src/agents/neural_agent.py -> ../../models
            model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../models'))
            model_path = os.path.join(model_dir, 'emotion_mlp_classifier.pkl')
            ...
            if os.path.exists(model_path):
                ...
                self.emotion_scaler = joblib.load(scaler_path)
                self.emotion_labels = joblib.load(labels_path)
                logger.info(f"Auditives System für Agent {self.agent_id} erfolgreich initialisiert (Modell geladen).")
            else:
                logger.warning(f"Kein trainiertes Emotionsmodell gefunden unter {model_path}. Auditive Wahrnehmung eingeschränkt.")
                
        except Exception as e:
            logger.error(f"Fehler bei der Initialisierung des auditiven Systems: {e}")

    def perceive_audio(self, audio_path: str) -> Dict[str, Any]:
        """
        Verarbeitet eine Audiodatei und erkennt die emotionale Färbung.
        
        Args:
            audio_path: Pfad zur Audiodatei
            
        Returns:
            Dict mit erkannter Emotion und Konfidenz
        """
        if not self.auditory_cortex or not self.emotion_model:
            logger.warning("Auditives System nicht bereit.")
            return {"emotion": "unknown", "confidence": 0.0}
            
        try:
            # 1. Laden und Vorverarbeiten
            audio, _ = self.audio_loader.load_audio(audio_path)
            cochleogram = self.auditory_features.compute_cochleogram(audio)
            
            # 2. Kortex-Verarbeitung
            module_outputs = self.auditory_cortex.process_input(cochleogram)
            
            # 3. Feature Extraktion (Pooling)
            feature_list = []
            min_time = float('inf')
            ...
            logger.info(f"Agent {self.agent_id} hört: {emotion} ({confidence:.2f})")
            
            # 5. Integration in das Überzeugungssystem (Beispiel)
            self._integrate_auditory_perception(emotion, confidence)
            ...
            logger.error(f"Fehler bei der auditiven Wahrnehmung: {e}")
            return {"emotion": "error", "details": str(e)}

    def _integrate_auditory_perception(self, emotion: str, confidence: float):
        """
        Integriert die gehörte Emotion in den internen Zustand des Agenten.
        """
        # Beispielhafte Reaktion auf Emotionen
        if emotion == 'anger':
            # Erhöhe wahrgenommene Bedrohung
            # Wir greifen direkt auf das Belief-System zu, falls vorhanden
            pass
            
        elif emotion == 'sadness':
            # Erhöhe Empathie / Sorge
            pass
    ...
    def _generate_cognitive_architecture(self) -> CognitiveArchitecture:
        ...
        # Kreative, offene Menschen neigen zu analogischem/assoziativem Denken
            ...
            primary = random.choice(
                [NeuralProcessingType.ANALOGICAL, NeuralProcessingType.ASSOCIATIVE]
            )
        # Neurotische Menschen neigen zu intuitivem/emotionalem Denken
        elif personality["neuroticism"] > 0.7:
            primary = random.choice(
                [NeuralProcessingType.INTUITIVE, NeuralProcessingType.EMOTIONAL]
            )
        else:
        ...
        if primary == NeuralProcessingType.SYSTEMATIC:
            ...
            secondary = random.choice(
                [NeuralProcessingType.SYSTEMATIC, NeuralProcessingType.ASSOCIATIVE]
            )
        else:
            all_types = [
            ...
            for other_name, (influence, polarity) in belief.connections.items():
                if other_name in self.beliefs and (belief_name, other_name) not in processed_pairs:
                    ...
                    if polarity < 0:
                        ...
                        dissonance += (
                            belief.strength
                            * self.beliefs[other_name].strength
                            * influence
                            * abs(polarity)
                            * certainty_weight
                        )
    ...
    def spreading_activation(self, seed_beliefs: List[str], activation_levels: List[float]):
        ...
        for _ in range(2):
            ...
            activations = {
                name: getattr(belief, "activation", 0.0) for name, belief in self.beliefs.items()
            }
                        ...
                        if conn_name in self.beliefs:
                            ...
                            if polarity < 0:
                                ...
                                if hasattr(self.beliefs[conn_name], "activation"):
                                    self.beliefs[conn_name].activation *= (
                                        1.0 - spread_activation * 0.3
                                    )
                            else:
                                # Aktivierung
                                if hasattr(self.beliefs[conn_name], "activate"):
                                    self.beliefs[conn_name].activate(
                                        spread_activation, self.current_time
                                    )
    ...
    def make_decision(
        self, scenario: "EthicalScenario", enhanced: bool = True
    ...
    def _original_make_decision(
        ...
        activation_levels = [
            scenario.relevant_beliefs[b] for b in seed_beliefs if b in self.beliefs
        ]
        ...
        context = {
            ...
            "emotional_valence": (
                np.mean(
                    [self.beliefs[b].emotional_valence for b in seed_beliefs if b in self.beliefs]
                )
                if seed_beliefs
                else 0
            ),
        ...
        for option_name, option_impacts in scenario.options.items():
            ...
            if "group_norms" in scenario.option_attributes.get(option_name, {}):
                for group, norm_alignment in scenario.option_attributes[option_name][
                    "group_norms"
                ].items():
            ...
            option_scores[option_name] = {"score": score, "justifications": justifications}
    ...
    def _finalize_decision(
        self, scenario: "EthicalScenario", option_scores: Dict[str, Dict]
    ) -> Dict:
        """
        ...
        if self.cognitive_architecture.primary_processing == NeuralProcessingType.SYSTEMATIC:
            ...
            availability_bias = self.cognitive_architecture.cognitive_biases.get(
            ...
            emotional_reactivity = self.cognitive_architecture.emotional_parameters.get(
        ...
        if len(final_scores) > 1:
            ...
            )  # Differenz zwischen höchstem und zweithöchstem
    ...
    def _calculate_consistency_boost(
        self, scenario: EthicalScenario, options: List[str], scores: List[float]
    ) -> np.ndarray:
    ...
    def _calculate_emotion_boost(
        self, scenario: EthicalScenario, options: List[str]
    ) -> np.ndarray:
        ...
        for i, option in enumerate(options):
            ...
            negativity_bias = self.cognitive_architecture.emotional_parameters.get(
                "negativity_bias", 0.6
            )
            if option_emotion < 0:
    ...
    def update_beliefs_from_experience(
        self, scenario: EthicalScenario, chosen_option: str
    ) -> Dict[str, float]:
        """
        ...
        if chosen_option in scenario.outcome_feedback:
            for belief_name, feedback in scenario.outcome_feedback[chosen_option].items():
                if belief_name in self.beliefs:
                    ...
                    if (
                        self.cognitive_architecture.primary_processing
                        == NeuralProcessingType.SYSTEMATIC
                    ):
                        learning_rate = (
                            base_learning_rate * 1.2
                        )  # Systematische Denker lernen mehr aus Erfahrung
                    elif (
                        self.cognitive_architecture.primary_processing
                        == NeuralProcessingType.ASSOCIATIVE
                    ):
                    ...
                    if (feedback > 0 and old_strength > 0.5) or (
                        feedback < 0 and old_strength < 0.5
                    ):
    ...
    def reinforcement_learn_from_outcome(
        self, scenario: EthicalScenario, chosen_option: str, outcome_feedback: Dict[str, float]
    ) -> Dict[str, float]:
        """
        ...
        experience = {
            "scenario_type": (
                scenario.scenario_id.split("_")[0]
                if "_" in scenario.scenario_id
        ...
        if total_reward > 0:
            ...
            self.rl_system["policy_gradient"][scenario_type][chosen_option] = min(
                0.9, current_prob + 0.05
            )
        elif total_reward < -0.1:
            current_prob = self.rl_system["policy_gradient"][scenario_type].get(chosen_option, 0.5)
            self.rl_system["policy_gradient"][scenario_type][chosen_option] = max(
                0.1, current_prob - 0.05
            )
        ...
        if len(recent_rewards) >= 5:
            ...
            if reward_variance > 0.5:  # Hohe Varianz = mehr Exploration
                self.rl_system["exploration_rate"] = min(
                    0.8, self.rl_system["exploration_rate"] + 0.02
                )
            else:  # Niedrige Varianz = weniger Exploration
                self.rl_system["exploration_rate"] = max(
                    0.05, self.rl_system["exploration_rate"] - 0.01
                )
    ...
    def multi_criteria_decision_making(
        self, scenario: EthicalScenario, option_evaluations: Dict[str, Dict[str, float]]
    ) -> Dict[str, float]:
        """
        ...
        for option_name, criteria_scores in option_evaluations.items():
            weighted_score = 0.0
            option_trade_offs: Dict[str, Any] = {}
            ...
            score_variance = (
                float(np.var(list(criteria_scores.values()))) if len(criteria_scores) > 1 else 0.0
            )
    ...
    def handle_uncertainty_and_ambiguity(
        self, scenario: EthicalScenario, belief_activations: Dict[str, float]
    ) -> Dict[str, float]:
        """
            ...
            if belief_name in self.beliefs:
                ...
                if total_uncertainty > tolerance:
                    ...
                    if self.uncertainty_handling["information_seeking_tendency"] > 0.6:
                        ...
                        info_gain = random.uniform(0.1, 0.3) * self.personality_traits.get(
                            "openness", 0.5
                        )
        ...
        avg_uncertainty = (
            np.mean(list(self.uncertainty_handling["uncertainty_estimates"].values()))
            if self.uncertainty_handling["uncertainty_estimates"]
            else 0.5
        )
    ...
    def advanced_social_learning(
        self, other_agents: List["NeuralEthicalAgent"], scenario_context: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
            ...
            for group_name, group_strength in self.group_identities.items():
                if group_name in other_agent.group_identities:
                    ...
                    if group_strength > 0.6 and other_group_strength > 0.6:
                        ...
                        conformity_pressure = self.social_learning["conformity_pressure"].get(
                            group_name, 0.3
                        )
    ...
    def _update_trust_and_reputation(self, other_agents: List["NeuralEthicalAgent"]):
        ...
        for other_agent in other_agents:
            ...
            new_reputation = current_reputation + reputation_learning_rate * (
                competence - current_reputation
            )
            self.social_learning["reputation_system"][agent_id] = np.clip(new_reputation, 0.0, 1.0)

    def _identify_domain_experts(
        self, other_agents: List["NeuralEthicalAgent"], scenario_context: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        ...
        domain = str(scenario_context.get("domain", "general"))
        ...
        for other_agent in other_agents:
            ...
            relevant_decisions = [
                ...
                if domain in str(decision.get("scenario_id", ""))
            ]
            ...
            if relevant_decisions:
                ...
                low_dissonance = np.mean(
                    [1.0 - d.get("cognitive_dissonance", 0.5) for d in relevant_decisions]
                )
            ...
            if (
                other_agent.cognitive_architecture.primary_processing
                == NeuralProcessingType.SYSTEMATIC
            ):
    ...
    def _calculate_social_proof(
        self, target_agent: "NeuralEthicalAgent", all_agents: List["NeuralEthicalAgent"]
    ) -> float:
        ...
        return float(1.0 + social_proof_sensitivity * agreement_ratio)
    ...
    def _calculate_agent_similarity(self, other_agent: "NeuralEthicalAgent") -> float:
        ...
        if (
            self.cognitive_architecture.secondary_processing
            == other_agent.cognitive_architecture.secondary_processing
        ):
        ...
        for foundation in self.moral_foundations:
            if foundation in other_agent.moral_foundations:
                diff = abs(
                    self.moral_foundations[foundation] - other_agent.moral_foundations[foundation]
                )
        ...
        total_similarity = (
            0.4 * personality_similarity + 0.3 * cognitive_similarity + 0.3 * moral_similarity
        )
    ...
    def _calculate_decision_similarity(self, other_agent: "NeuralEthicalAgent") -> float:
        ...
        other_scenarios = {
            d["scenario_id"]: d["chosen_option"] for d in other_agent.decision_history
        }
    ...
    def _estimate_agent_competence(self, other_agent: "NeuralEthicalAgent") -> float:
        ...
        avg_confidence = np.mean(
            [abs(d.get("confidence", 0.5)) for d in other_agent.decision_history]
        )

        # Niedrige kognitive Dissonanz als Zeichen für Kompetenz
        avg_dissonance = np.mean(
            [d.get("cognitive_dissonance", 0.5) for d in other_agent.decision_history]
        )
        ...
        competence = 0.4 * avg_confidence + 0.3 * low_dissonance_score + 0.3 * consistency_score
    ...
    def _calculate_decision_consistency(self, other_agent: "NeuralEthicalAgent") -> float:
        ...
        scenario_decisions: Dict[str, List[Any]] = {}
        for decision in other_agent.decision_history:
            scenario_type = (
                decision["scenario_id"].split("_")[0]
                if "_" in decision["scenario_id"]
                else decision["scenario_id"]
            )
    ...
    def _learn_from_agent(
        self, other_agent: "NeuralEthicalAgent", influence_strength: float
    ) -> Dict[str, float]:
    ...
    def temporal_belief_dynamics(self, time_step: float = 1.0) -> Dict[str, float]:
        ...
        for belief_name, belief in self.beliefs.items():
            ...
            belief_momentum = cast(Dict[str, float], self.temporal_dynamics["belief_momentum"])
            if belief_name in belief_momentum:
                momentum = belief_momentum[belief_name]
                momentum_effect = momentum * 0.1 * time_step
                temporal_change += momentum_effect

                # Momentum verlangsamt sich über Zeit (Reibung)
                belief_momentum[belief_name] *= 0.95
            ...
            # Habituation: Reduzierte Reaktion auf
            habituation_effects = cast(Dict[str, float], self.temporal_dynamics["habituation_effects"])
            if belief_name in habituation_effects:
                habituation_level = habituation_effects[belief_name]
    ...
    def metacognitive_monitoring(
        self, decision_result: Dict[str, Union[str, float]]
    ) -> Dict[str, float]:
        """
        ...
        cognitive_dissonance = float(decision_result.get("cognitive_dissonance", 0.0))
        confidence = abs(float(decision_result.get("confidence", 0.5)))
        ...
        if error_detected and monitoring_level > 0.6:
            ...
            if error_indicator > switch_threshold:
                ...
                if error_indicator > 0.8:  # Schwerwiegender Fehler
                    ...
                    adjustments["exploration_increase"] = (
                        self.rl_system["exploration_rate"] - old_exploration
                    )
                    ...
                    adjustments["learning_rate_increase"] = (
                        self.rl_system["learning_rate"] - old_learning_rate
                    )
        ...
        recent_decisions = len(
            [d for d in self.decision_history if self.current_time - d["timestamp"] < 3.0]
        )

        cognitive_load = (
            num_active_beliefs / 20.0 + working_memory_load + recent_decisions / 10.0
        ) / 3.0
        ...
        if cognitive_load > 0.8 and load_awareness > 0.6:
            ...
            if self.cognitive_architecture.primary_processing == NeuralProcessingType.SYSTEMATIC:
                ...
                adjustments["load_reduction"] = (
                    old_balance - self.cognitive_architecture.processing_balance
                )

            # Arbeitsgedächtnis leeren
            overflow = max(
                0, len(self.working_memory["contents"]) - self.working_memory["capacity"]
            )
            if overflow > 0:
        ...
        if len(self.decision_history) >= 5:
            ...
            recent_decisions_list = self.decision_history[-5:]

            # Varianz in Konfidenzlevels
            confidences = [abs(d.get("confidence", 0.5)) for d in recent_decisions_list]
            ...
            dissonances = [d.get("cognitive_dissonance", 0.0) for d in recent_decisions_list]
            ...
            if avg_dissonance > 0.6:  # Anhaltend hohe Dissonanz
                ...
                adjustments["improved_error_detection"] = (
                    old_detection - self.metacognition["error_detection"]
                )
    ...
    def enhanced_make_decision(
        ...
        context = {
            ...
            "domain": (
                scenario.scenario_id.split("_")[0] if "_" in scenario.scenario_id else "general"
            ),
        }

        # Verarbeitung durch kognitive Architektur
        processed_beliefs = self.cognitive_architecture.process_information(
            adjusted_beliefs, context
        )
    ...
    def learn_from_decision_outcome(
        ...
        other_agents: Optional[List["NeuralEthicalAgent"]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        rl_learning = self.reinforcement_learn_from_outcome(
            scenario, chosen_option, outcome_feedback
        )
        ...
        for criterion in self.decision_criteria["utility_weights"]:
            if criterion.replace("_", "").replace("minimization", "outcome") in outcome_feedback:
                criterion_outcome = outcome_feedback.get(
                    criterion.replace("_", "").replace("minimization", "outcome"), 0
                )
        ...
        actual_surprise = (
            abs(total_outcome) if abs(total_outcome) > 0.5 else 1.0 - abs(total_outcome)
        )
        ...
        # === SOCIAL LEARNING ===
        if other_agents:
            social_learning_changes = self.advanced_social_learning(
                other_agents, {"domain": scenario.scenario_id.split("_")[0]}
            )
        ...
        for belief_name, change in belief_updates.items():
            if abs(change) > 0.05:  # Signifikante Änderung
                belief_momentum = cast(Dict[str, float], self.temporal_dynamics["belief_momentum"])
                current_momentum = belief_momentum.get(belief_name, 0.0)
                # Momentum in Richtung der Änderung verstärken
                new_momentum = current_momentum + np.sign(change) * min(0.3, abs(change))
                belief_momentum[belief_name] = np.clip(
                    new_momentum, -0.5, 0.5
                )
        ...
        recent_similar = [
            d for d in self.decision_history[-10:] if scenario_type in d.get("scenario_id", "")
        ]
        ...
        if len(recent_similar) > 3:  # Häufiges ähnliches Szenario
            ...
            for belief_name in scenario.relevant_beliefs:
                if belief_name in self.beliefs:
                    current_habituation = habituation_effects.get(
                        belief_name, 0.0
                    )
                    habituation_effects[belief_name] = min(
                        0.9, current_habituation + 0.1
                    )
        ...
        if learning_effectiveness > 0.6:  # Gutes Ergebnis
            ...
            current_lr = float(self.rl_system["learning_rate"])
            self.rl_system["learning_rate"] = min(0.3, current_lr * 1.05)
            
            current_ar = float(self.temporal_dynamics["adaptation_rate"])
            self.temporal_dynamics["adaptation_rate"] = min(
                0.3, current_ar * 1.05
            )
        elif learning_effectiveness < 0.3:  # Schlechtes Ergebnis
            # Reduziere Lernparameter (konservativer werden)
            current_lr = float(self.rl_system["learning_rate"])
            self.rl_system["learning_rate"] = max(0.01, current_lr * 0.95)
            
            current_ar = float(self.temporal_dynamics["adaptation_rate"])
            self.temporal_dynamics["adaptation_rate"] = max(
                0.05, current_ar * 0.95
            )
                ...
                for connected_belief, (influence, polarity) in belief.connections.items():
                    if connected_belief in self.beliefs:
                        ...
                        if (
                            self.cognitive_architecture.primary_processing
                            == NeuralProcessingType.ASSOCIATIVE
                            or self.cognitive_architecture.secondary_processing
                            == NeuralProcessingType.ASSOCIATIVE
                        ):
                            ...
                            for concept in belief.associated_concepts:
                                if concept in self.beliefs[connected_belief].associated_concepts:
                                    assoc_strength = (
                                        belief.associated_concepts[concept]
                                        * self.beliefs[connected_belief].associated_concepts[
                                            concept
                                        ]
                                    )
