...
class NeuralEthicalAgent:
    ...
    def multi_criteria_decision_making(
        ...
        for option_name, criteria_scores in option_evaluations.items():
            ...
            score_variance = (
                np.var(list(criteria_scores.values())) if len(criteria_scores) > 1 else 0
            )
    ...
    def advanced_social_learning(
        self, other_agents: List["NeuralEthicalAgent"], scenario_context: Dict[str, float] = None
    ) -> Dict[str, float]:
        """
    ...
    def learn_from_decision_outcome(
        ...
        other_agents: List["NeuralEthicalAgent"] = None,
    ) -> Dict[str, float]:
        """
        ...
        for belief_name, change in belief_updates.items():
            if abs(change) > 0.05:  # Signifikante Änderung
                ...
                self.temporal_dynamics["belief_momentum"][belief_name] = np.clip(
        ...
        if len(recent_similar) > 3:  # Häufiges ähnliches Szenario
            for belief_name in scenario.relevant_beliefs:
                if belief_name in self.beliefs:
                    current_habituation = self.temporal_dynamics["habituation_effects"].get(
                        belief_name, 0.0
                    )
                    self.temporal_dynamics["habituation_effects"][belief_name] = min(
        ...
        if learning_effectiveness > 0.6:  # Gutes Ergebnis
            ...
            self.temporal_dynamics["adaptation_rate"] = min(
                0.3, self.temporal_dynamics["adaptation_rate"] * 1.05
            )
            ...
            self.temporal_dynamics["adaptation_rate"] = max(
                0.05, self.temporal_dynamics["adaptation_rate"] * 0.95
            )
