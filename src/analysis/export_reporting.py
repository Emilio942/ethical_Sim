    ...
    def _export_agents_csv(agents_data: List[Dict], filepath: str) -> None:
        ...
        for agent in agents_data:
            row = {
                ...
                "avg_decision_value": (
                    np.mean([d.get("value", 0.5) for d in agent.get("decision_history", [])])
                    if agent.get("decision_history")
                    else 0.0
                ),
                "avg_confidence": (
                    np.mean([d.get("confidence", 0.5) for d in agent.get("decision_history", [])])
                    if agent.get("decision_history")
                    else 0.0
                ),
            }
...
"""
        ...
        if hasattr(society, "social_network"):
            G = society.social_network
            report += (
                f"- **Social Network**: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges\n"
            )
                ...
                if hasattr(agent, "decision_history"):
                    ...
                    if decision_count > 0:
                        ...
                        avg_confidence = np.mean(
                            [d.get("confidence", 0.5) for d in agent.decision_history]
                        )
        ...
        if validation_data:
            ...
            if total_checks > 0:
                ...
                report += (
                    f"- **Overall Pass Rate**: {pass_rate:.1%} ({passed_checks}/{total_checks})\n"
                )
...
"""
        ...
        if hasattr(agent, "decision_history") and agent.decision_history:
            ...
            if len(decisions) >= 10:
                ...
                older_values = (
                    decision_values[-20:-10] if len(decisions) >= 20 else decision_values[:-10]
                )
...
class AutomatedReporter:
    ...
    def generate_comprehensive_report(
        self, society, metrics_data=None, validation_data=None
    ) -> Dict[str, str]:
        ...
        # Simulation summary
        summary_report = ReportGenerator.generate_simulation_summary(
            society, metrics_data, validation_data
        )
        ...
        if hasattr(society, "agents"):
            ...
            for i, agent in enumerate(society.agents):
                agent_id = f"agent_{i}"
                agent_metrics = (
                    metrics_data.get("agents", {}).get(agent_id) if metrics_data else None
                )

                agent_report = ReportGenerator.generate_agent_analysis(
                    agent, agent_id, agent_metrics
                )
