    ...
    def validate_belief_consistency(agent) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            details={
                ...
                'belief_range': (min(belief_values), max(belief_values)) if belief_values else (0, 0)
            },
            timestamp=datetime.now()
        )
    ...
    def validate_decision_plausibility(agent) -> ValidationResult:
        ...
        for decision in recent_decisions:
            ...
            if hasattr(agent, 'beliefs') and agent.beliefs.beliefs:
                relevant_beliefs = [v for k, v in agent.beliefs.beliefs.items() 
                                  if any(keyword in scenario.lower() 
                                        for keyword in k.lower().split('_'))]
    ...
    def validate_learning_progress(agent) -> ValidationResult:
        ...
        if not hasattr(agent, 'decision_history') or len(agent.decision_history) < 5:
            return ValidationResult(
                ...
                details={'decision_count': len(agent.decision_history) if hasattr(agent, 'decision_history') else 0},
                timestamp=datetime.now()
            )
        ...
        if len(decision_values) >= 10:
            ...
            older_values = decision_values[-20:-10] if len(decision_values) >= 20 else decision_values[:-10]
...
class SocietyValidator:
    ...
    def validate_social_stability(society) -> ValidationResult:
        ...
        if len(society.agents) >= 2:
            positive_count = sum(1 for agent in society.agents 
                               if hasattr(agent, 'beliefs') and agent.beliefs.beliefs
    ...
    def detect_extreme_behaviors(society) -> List[ValidationResult]:
        ...
        for i, agent in enumerate(society.agents):
            ...
            if z_score > 2.5:  # More than 2.5 standard deviations
                anomalies.append(ValidationResult(
...
class ValidationSuite:
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        # Critical issues
        critical_issues = [r for r in self.validation_history 
                          if r.severity == ValidationSeverity.CRITICAL or 
    ...
    def export_validation_results(self, filepath: str, format: str = 'json') -> None:
        ...
        if format.lower() == 'json':
            ...
            import csv
            if export_data:
                fieldnames = ['check_name', 'validation_type', 'severity', 'passed', 
